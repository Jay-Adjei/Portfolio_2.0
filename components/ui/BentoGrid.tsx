...
export const BentoGridItem = ({
  ...
  return (
    <div
      className={cn(
        "row-span-1 overflow-hidden rounded-3xl relative group/bento hover:shadow-xl transition duration-200 shadow-input dark:shadow-none p-4 justify-between flex flex-col space-y-4 border border-white/[0.1]",
        className
      )}
      style={{
        ...
        <div className={`${id === 6} && 'flex justify-center h-full `}>
            ...
            <div className='flex gap-1 lg:gap-5 w-fit absolute -right-3 lg:-right-2'>
              <div className='flex flex-col gap-3 lg:gap-8'>
                {['ReactNative', 'React.js', 'TypeScript',].map((item) => (
              ...
              <div className='flex flex-col gap-3 lg:gap-8'>
              <span className='py-4 px-3 rounded-lg text-center bg-[#10132e]'/>
                {['MongoDB', 'FireBase', 'Next.js'].map((item) => (
                  <span key={item} className="py-2 lg:py-4 lg;px-3 px-3 text-xs 
                    lg:text-base opacity-50 lg:opacity-100 rounded-lg text-center bg-[#10132E]">
                    {item}
                  </span>
